...
class EnhancedAIDetection:
    ...
    def _setup_keyword_structures(self):
        ...
        "responsible ai", "ethical ai", "model governance",
    ...
    def filter_ai_false_positives(self, df: pd.DataFrame) -> pd.DataFrame:
        """Remove comprehensive AI false positives focusing on main patterns"""
        print("🔍 Filtering AI false positives...")
        
        initial_count = len(df)
        keywords_col = 'ai_keywords_str' if 'ai_keywords_str' in df.columns else 'matched_keywords'
        ...
        for idx, row in df.iterrows():
            ...
            # Only check jobs that include 'ai' keyword
            if 'ai' in keywords_list:
                reason = self._check_ai_false_positive_focused(content, keywords_list)
                if reason:
                    keep_mask[idx] = False
        ...
        print(f"✅ Removed {removed_count:,} AI false positives")
        print(f"   ({removed_count/initial_count*100:.1f}% of original dataset)")
        print(f"   Remaining: {len(filtered_df):,} jobs")
        
        # Show breakdown of removal reasons
        if removal_reasons:
    ...
    def _check_ai_false_positive_focused(self, content: str, keywords_list: List[str]) -> str:
        """Check for main AI false positive patterns based on analysis"""
        
        content_lower = content.lower()
        
        # 1. Italian false positives - both single 'ai' and mixed content
        if self._is_italian_ai_false_positive(content_lower, keywords_list):
            return "Italian language"
        
        # 2. French "j'ai" false positives  
        if re.search(r"\bj'ai\b", content_lower):
            return "French j'ai"
        
        # 3. Only for single 'ai' keyword: check if it's in legitimate AI context
        if len(keywords_list) == 1 and keywords_list[0] == 'ai':
            if not self._has_legitimate_ai_context(content_lower):
                return "Isolated ai without AI context"
        
        return None
    
    def _is_italian_ai_false_positive(self, content_lower: str, keywords_list: List[str]) -> bool:
        """Detect Italian AI false positives with improved accuracy"""
        
        # Italian function words and common patterns
        it_indicators = len(re.findall(r'\b(il|la|le|con|per|di|e|sono|hanno|su|in|ai|hai|mai|dai|alla|nel|del|che|una|uno|questa|questo|suoi|nostri|loro)\b', content_lower))
        
        # Non-Italian indicators (reduce false negatives for legitimate AI)
        non_it_indicators = len(re.findall(r'\b(the|and|with|for|is|are|have|has|data|software|technology|solution|development|engineer|machine|learning|artificial|intelligence)\b', content_lower))
        
        # Strong Italian indicators
        strong_it_patterns = [
            r'\b(assicura|assicurazione|invalidità|federale|cantone|ufficio|segreteria|amministrazione)\b',
            r'\b(migrazione|collaboratori|candidatura|curriculum|vitae)\b',
            r'\b(presso|inoltre|attraverso|durante|sempre)\b'
        ]
        
        strong_it_score = sum(len(re.findall(pattern, content_lower)) for pattern in strong_it_patterns)
        
        # Decision logic
        if strong_it_score > 0 and it_indicators > non_it_indicators:
            return True
        elif it_indicators > (non_it_indicators * 2):  # Strong Italian dominance
            return True
        
        return False
    
    def _has_legitimate_ai_context(self, content_lower: str) -> bool:
        """Check if 'ai' appears in legitimate AI/tech context"""
        
        # Find all 'ai' occurrences with context
        ai_contexts = re.findall(r'.{0,60}\bai\b.{0,60}', content_lower)
        
        for context in ai_contexts:
            # Legitimate AI indicators
            legitimate_patterns = [
                r'\b(data.*ai|ai.*data|daten.*ai|ai.*daten)\b',
                r'\b(artificial.*intelligence|machine.*learning)\b', 
                r'\b(ai.*engineer|ai.*developer|ai.*solution)\b',
                r'\b(cloud.*ai|ai.*cloud|iot.*ai|ai.*iot)\b',
                r'\b(ai.*tech|tech.*ai|ai.*platform|platform.*ai)\b',
                r'\b(digital.*ai|ai.*digital|software.*ai|ai.*software)\b'
            ]
            
            if any(re.search(pattern, context) for pattern in legitimate_patterns):
                return True
    ...
    def export_results(self, ai_jobs: pd.DataFrame, dataset_type="train") -> str:
        ...
        output_path = self.data_dir / f"enhanced_ai_detection_{dataset_type}.csv"
...
def filter_false_positives_from_dataset(input_filename):
    """Filter comprehensive AI false positives from existing AI jobs dataset"""
    
    input_file = f"Data/{input_filename}"
    # Generate output filename by replacing suffix
    if input_filename.endswith('_WITH_FALSE_POSITIVES.csv'):
        output_file = f"Data/{input_filename.replace('_WITH_FALSE_POSITIVES.csv', '_CLEAN.csv')}"
    else:
        base_name = input_filename.replace('.csv', '')
        output_file = f"Data/{base_name}_CLEAN.csv"
    
    print("🔍 Filtering comprehensive AI false positives")
    print("=" * 50)
    ...
    if keywords_col not in df.columns:
        print(f"❌ Error: Neither 'ai_keywords_str' nor 'matched_keywords' column found in dataset")
        print(f"Available columns: {list(df.columns)}")
        return
    
    # Filter false positives
    filtered_df = detector.filter_ai_false_positives(df)
    ...
    print(f"Filtered: {len(filtered_df):,} jobs") 
    print(f"Removed: {removed_count:,} false positives ({removed_count/len(df)*100:.1f}%)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--test":
            ...
            run_both_sets()
        elif sys.argv[1] == "--filter-false-positives":
            if len(sys.argv) > 2:
                filter_false_positives_from_dataset(sys.argv[2])
            else:
                print("Error: Please provide dataset filename")
                print("Usage: python3 stage_1_keyword_match.py --filter-false-positives <dataset_file>")
        else:
            print("Usage:")
            print("  python3 stage_1_keyword_match.py                                    # Process training set only")
            print("  python3 stage_1_keyword_match.py --test                             # Process test set only") 
            print("  python3 stage_1_keyword_match.py --both                             # Process both sets")
            print("  python3 stage_1_keyword_match.py --filter-false-positives <file>   # Filter Italian AI false positives")
    else:
