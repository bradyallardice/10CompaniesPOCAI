...
class ONETSimilarityMatcher:
    ...
    def compute_similarities_and_filter(self, 
        ...
        for i, app_text in enumerate(unique_apps):
            ...
            for onet_idx in selected_indices:
                for job_uid in matching_jobs:
                    # Get timestamp from apps_df for this job_uid
                    job_info = apps_df[apps_df['job_uid'] == job_uid]
                    timestamp = job_info['first_occurrence_tst_created'].iloc[0] if len(job_info) > 0 else None
    ...
    def validate_with_cross_encoder(self, 
                                  ...
                                  threshold: float = 0.6,
                                  batch_size: int = 16) -> Tuple[pd.DataFrame, dict]:
        """
        Phase E: Validate top similarity matches using cross-encoder.
        ...
        for _, row in similarity_df.iterrows():
            pairs.append([row['app_text'], row['onet_task']])
    ...
    def run_full_pipeline(self, 
                         ...
                         cross_encoder_batch_size: int = 16) -> Tuple[pd.DataFrame, dict]:
        """
        ...
        job_mapping_agg = results_df.groupby('app_text').agg({
            'job_uid': lambda x: list(set(x)),  # Unique job_uids
        ...
        job_mapping['num_jobs'] = job_mapping['job_uid'].apply(len)
        ...
        if all_similarities_df is not None:
            ...
            all_similarities_file = os.path.join(output_dir, f"all_similarity_scores.csv")
...
def parse_arguments():
    ...
    parser.add_argument("--use-cross-encoder", action="store_true",
                       help="Use cross-encoder for validation of top matches")
    ...
    parser.add_argument("--cross-encoder-threshold", type=float, default=0.6,
                       help="Cross-encoder threshold for filtering matches (default: 0.6)")
    
    parser.add_argument("--cross-encoder-batch-size", type=int, default=16,
                       help="Batch size for cross-encoder inference (default: 16)")
