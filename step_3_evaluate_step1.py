...
class Step1PromptExecutor:
    ...
    def __init__(self, api_key: str = None, model: str = "gpt-4o-mini", max_retries: int = 3):
        """
    ...
    def get_step1_prompt(self):
        """
        Get the exact step 1 system prompt from step_3_extract_ai_tasks.py
...
"""
    ...
    def execute_step1_prompt(self, job_content: str) -> Dict[str, List[str]]:
        """
        ...
        """
        system_prompt = self.get_step1_prompt()
        user_prompt = job_content
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"All attempts failed for job content: {job_content[:100]}...")
    ...
    def _convert_step1_to_evaluation_format(self, step1_response: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            # Extract ai_capability values from ai_application_tasks
            ai_tasks = parsed.get('ai_application_tasks', [])
            capabilities = []
    ...
    def process_dataset(self, df: pd.DataFrame, content_column: str = 'content_clean') -> pd.DataFrame:
        """
        ...
        """
        logger.info(f"Processing {len(df)} job advertisements with step 1 prompt")
        ...
        for idx, row in df.iterrows():
            ...
            # Execute step 1 prompt
            prediction = self.execute_step1_prompt(str(job_content))
        ...
        logger.info(f"Completed processing {len(df)} job advertisements")
        return result_df
...
class PerformanceTracker:
    ...
    def log_run_result(self, prompt_id: str, run_index: int, metrics: Dict[str, float], 
                      threshold: float, dataset: str = "test", notes: str = ""):
        """
        ...
        if not os.path.exists(self.runs_file):
            ...
            run_df.to_csv(self.runs_file, mode='a', header=False, index=False)
    ...
    def log_prompt_summary(self, prompt_id: str, all_run_metrics: List[Dict[str, float]], 
                          threshold: float, dataset: str = "test"):
        """
        ...
        if not os.path.exists(self.prompts_file):
            summary_df.to_csv(self.prompts_file, index=False)
        else:
            summary_df.to_csv(self.prompts_file, mode='a', header=False, index=False)
...
def evaluate_single_dataset(dataset_df: pd.DataFrame, dataset_name: str, args, 
                           ...
                           step1_executor: Optional['Step1PromptExecutor'] = None) -> List[Dict[str, float]]:
    """
    ...
    if args.run_step1 and step1_executor:
        ...
        current_df = step1_executor.process_dataset(current_df, args.content_column)
    ...
    tracker.log_prompt_summary(args.prompt_id, all_run_metrics, evaluator.similarity_threshold, dataset_name)
...
def main():
    ...
    if args.run_step1:
        ...
        try:
            step1_executor = Step1PromptExecutor(model=args.openai_model)
        except Exception as e:
    ...
    # Evaluate each requested dataset
    all_results = {}
